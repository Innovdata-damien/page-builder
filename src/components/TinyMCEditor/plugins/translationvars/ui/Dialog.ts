...
const dialogSpec = (bodyItems: any, initialData: any, editor: EditorTinymce, options: Options) => ({
    //(options.translationVars ? [...options.translationVars] : [])
    ...
    body: {
        ...
        items: [
            ...
            },
            {
                ...
                name: 'transvars_list', // identifier
            }
        ]
    },
    initialData,
...
const getTransvarsList = (translationVars) =>{
    translationVars.map(item =>{
        console.log(item)
        item.icon = item.text;
        return item;
    })
...
const open = function (editor: EditorTinymce, options: Options) {
    ...
    console.log(getTransvarsList(options.translationVars ))
    const initialState = {
        transvars_list: options.translationVars ? options.translationVars : []
    };
    const dialogApi: any = editor.windowManager.open(dialogSpec([], initialState, editor, options));
    //dialogApi.block('Loading...');
